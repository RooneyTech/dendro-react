...
// Type-check gate: webpack and the harness suites both run without tsc, so a
// type error can slip through an otherwise-green merge (it did — #47's
// fixtures). Fail fast here so compile is part of the same gate every merge
// already runs.
process.stdout.write('  typecheck (npm run compile) ... ');
try {
  execSync('npx tsc -p .', { encoding: 'utf-8', timeout: 180000, cwd: path.join(__dirname, '..') });
  console.log('\x1b[32mok\x1b[0m');
} catch (e) {
  console.log('\x1b[31mFAILED\x1b[0m');
  console.log((e.stdout || e.message || '').toString().split('\n').slice(0, 15).join('\n'));
  console.log('\nFix type errors before running suites.');
  process.exit(1);
}
console.log();
