...
## What this is
VS Code extension + MCP server for React and React Native codebase visualization. OXC-based static parser + D3 dendrogram webview + 56 MCP tools for AI-driven component analysis. Live runtime introspection via React DevTools. Marketplace: Dendro React (free — all 56 tools).
...
- **Licensing:** dormant as of v0.5.1 — all 56 tools free (`isGated()` returns false in `src/mcp/pro-gate.ts`). HMAC-SHA256 license plumbing retained in `src/licensing/` for a possible future paid tier.
